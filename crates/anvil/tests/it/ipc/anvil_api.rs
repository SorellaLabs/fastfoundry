...
async fn test_can_set_storage_bsc_fork() {
    let (api, handle) =
        spawn(NodeConfig::test_ipc().with_eth_rpc_url(Some("https://bsc-dataseed.binance.org/")))
            .await;
