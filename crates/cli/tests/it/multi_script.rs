...
forgetest_async!(
    ...
    |prj: TestProject, cmd: TestCommand| async move {
        let (api1, handle1) = spawn(NodeConfig::test_http()).await;
        let (api2, handle2) = spawn(NodeConfig::test_http()).await;
...
forgetest_async!(
    ...
    |prj: TestProject, cmd: TestCommand| async move {
        let (_, handle1) = spawn(NodeConfig::test_http()).await;
        let (_, handle2) = spawn(NodeConfig::test_http()).await;
        let mut tester = ScriptTester::new_broadcast_without_endpoint(cmd, prj.root());
